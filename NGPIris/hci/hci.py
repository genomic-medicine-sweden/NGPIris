...
class HCIManager:
	...
	def generate_token(self, password):
	    ...
	    my_key = requests.post(f"https://{self.address}:{self.authport}/auth/oauth/", data={"grant_type": "password", "username": "admin", "password": f"{admin_pass}", "scope": "*", 
	...
	def index_lister(self, token, index="all"):
	    if index == "all":
